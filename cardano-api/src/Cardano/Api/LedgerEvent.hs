...
data LedgerEvent
  ...
  | -- | Pools have been reaped and deposits refunded.
    ...
    -- | A number of Plutus script evaluations that all succeeded.
  | SuccessfulPlutusScript (NonEmpty PlutusDebug)
    -- | A number of Plutus script evaluations, some of which failed.
...
instance {-# OVERLAPPING #-} ConvertLedgerEvent (ShelleyBlock protocol (AlonzoEra StandardCrypto))
  ...
  toLedgerEvent evt = case unwrapLedgerEvent evt of
    ShelleyLedgerEventBBODY ( ShelleyInAlonzoEvent
        ( LedgersEvent
            ( Shelley.LedgerEvent
                ( Shelley.UtxowEvent _ )))) -> undefined
