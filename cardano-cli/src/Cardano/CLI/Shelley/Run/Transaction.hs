...
runTxBuild (AnyCardanoEra era) (AnyConsensusModeParams cModeParams) networkId mScriptValidity
  ...
  let localNodeConnInfo = LocalNodeConnectInfo cModeParams networkId sockPath
      ...
      -- TODO: How can we make it fool proof that we always get all the txins
  ...
  case (consensusMode, cardanoEraStyle era) of
    (CardanoMode, ShelleyBasedEra sbe) -> do
      ...
      liftIO $ print $ "TxBody content: " <> (show txBodyContent :: String)
      ...
      (BalancedTxBody balancedTxBody _ fee, eUnits, substitutedTxBody) <-
      ...
      putStrLn ("" :: String)
      putStrLn $ "Before balancing: " <> (show txBodyContent :: String)
      putStrLn ("" :: String)
      putStrLn $ "Exec units map: " <> (show eUnits :: String)
      putStrLn ("" :: String)
      putStrLn $ "Exec units subst TxBody: " <> (show substitutedTxBody :: String)

      putStrLn ("" :: String)
      putStrLn $ "Estimated transaction fee: " <> (show fee :: String)
      putStrLn ("" :: String)
      putStrLn $ "Tx: " <> (show balancedTxBody :: String)
...
validateTxInsReference era txins =
 ...
 where
  convert
    :: (TxIn, Maybe (ScriptWitnessFiles WitCtxTxIn))
    -> ExceptT ShelleyTxCmdError IO (Maybe ReferenceInput)
      ...
      Just scriptWitnessFiles -> do
        ...
        case sWit of
          PlutusScriptWitness _ _ (PReferenceScript refIn) _ _ _ ->
            return $ Just $ ReferenceInput refIn
